...
void ICM20948::setAccelSensitivity(AccelSensitivity s) {
	...
	switch (s) {
		case ACCEL_RANGE_2G: accelScale = 16384.f; break;
		case ACCEL_RANGE_4G: accelScale = 8192.f; break;
		case ACCEL_RANGE_8G: accelScale = 4096.f; break;
		case ACCEL_RANGE_16G: accelScale = 2048.f; break;
	}
}
...
void ICM20948::setGyroSensitivity(GyroSensitivity s) {
	...
	switch (s) {
		case GYRO_RANGE_250DPS: gyroScale = 131.f; break;
		case GYRO_RANGE_500DPS: gyroScale = 65.5; break;
		case GYRO_RANGE_1000DPS: gyroScale = 32.8; break;
		case GYRO_RANGE_2000DPS: gyroScale = 16.4; break;
	}
}
...
void ICM20948::init() {
	...
	switchBank(0);
	spi.write<uint8_t>(REG::USER_CTRL,
					   0x30);  // DMP disabled, fifo disabled, i2c master enabled, SPI mode.
...
void ICM20948::externalI2CStartTransaction(uint8_t addr, uint8_t reg, bool isRead) {
	switchBank(3);
...
void ICM20948::magnetometerWrite(uint8_t reg, uint8_t data) {
	...
	bool ok = externalI2CWaitComplete(100);	 // todo do something with status
	if (!ok) {
		ESP_LOGE(TAG, "magnetometer write NACKed: %02hhx %02hhx", reg, data);
	}
}
