...
void ICM20948::setSleep(bool sleep)
{
	...
	spi.write8(0x06, 0x01 | (sleep ? 0x40 : 0x00));
}

void ICM20948::init()
{
	switchBank(0);
	spi.write8(0x06, 0x80); // reset
	vTaskDelay(100 / portTICK_PERIOD_MS);

	uint8_t id = spi.read8(0x00);
	if (id != 0xea) {
		ESP_LOGW(TAG, "ICM module reports wrong id %02hhx", id);
	}

	spi.write8(0x03, 0x30); // DMP disabled, fifo disabled, i2c master enabled, SPI mode.
