...
static const adc_bits_width_t ADC2_BIT_WIDTH = ADC_WIDTH_BIT_13;
...
ADC1::ADC1():
	characteristics(new esp_adc_cal_characteristics_t())
{
	...
	esp_adc_cal_characteristics_t* c = reinterpret_cast<esp_adc_cal_characteristics_t*>(characteristics);
	esp_adc_cal_value_t mode = esp_adc_cal_characterize(
			ADC_UNIT_1, ADC_ATTEN_DB_11, ADC1_BIT_WIDTH,
			1100, c);
	...
	ESP_LOGI(TAG, "ADC1 characteristics: a = %u, b = %u, vref=%umV data %p %p",
			c->coeff_a, c->coeff_b, c->vref, c->low_curve, c->high_curve);
...
int ADC1::read(uint8_t ch)
{
	...
	uint32_t mVolts = esp_adc_cal_raw_to_voltage(v,
